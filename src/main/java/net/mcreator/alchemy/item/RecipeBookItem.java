...
import net.mcreator.alchemy.gui.FusionGuideGuiWindow;
...
public class RecipeBookItem extends AlchemyModElements.ModElement {
	@ObjectHolder("alchemy:recipe_book")
	public static final Item block = null;
	public RecipeBookItem(AlchemyModElements instance) {
		super(instance, 2);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof FusionGuideGuiWindow) {
	...
	public static class ItemCustom extends Item {
		public ItemCustom() {
			super(new Item.Properties().group(AlchemyItemGroup.tab).maxStackSize(1).isImmuneToFire().rarity(Rarity.EPIC));
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new FusionGuideGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
