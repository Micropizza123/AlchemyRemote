...
package net.mcreator.alchemy.keybind;
...
import net.mcreator.alchemy.procedures.AbilitiesOnKeyPressedProcedure;
...
public class AbilitiesKeyBinding extends AlchemyModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.alchemy.abilities", GLFW.GLFW_KEY_R, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					AlchemyMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				AbilitiesOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
